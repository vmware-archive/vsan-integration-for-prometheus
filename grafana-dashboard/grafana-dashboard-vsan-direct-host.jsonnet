...
local panels =
  ReadWriteLayersPanels(10, step * 0, "Latency", "s",
    "sum(rate(vmware_vsan_disks_dev_duration_seconds_total{storagepool=~\"storagepool-vsandirect.*\",io_type=\"LSOMIOTYPE\"}[1m])) by (hostname, io_type) / sum(rate(vmware_vsan_disks_dev_io_total{storagepool=~\"storagepool-vsandirect.*\", io_type=\"LSOMIOTYPE\"}[1m])) by (hostname, io_type)") +
  ReadWriteLayersPanels(30, step * 1, "IOPS", "short",
    "sum(rate(vmware_vsan_disks_dev_io_total{storagepool=~\"storagepool-vsandirect.*\",io_type=\"LSOMIOTYPE\"}[1m])) by (hostname, io_type)") +
  ReadWriteLayersPanels(50, step * 2, "Tput", "Bps",
    "sum(rate(vmware_vsan_disks_dev_bytes_total{storagepool=~\"storagepool-vsandirect.*\",io_type=\"LSOMIOTYPE\"}[1m])) by (hostname, io_type)"
);
